...
- Public activation request form at `https://sua.actuallyleviticus.xyz/request`. A request records one or more selected areas, an RA1/RA2/RA3 category, UTC start/end, the requester's name or VATSIM CID, contact email, and required activation details explaining why the airspace is needed without activating it automatically. RA1 is shown in green, RA2 in yellow, and RA3 in red. Controllers review pending requests from the `REQUESTS` button on the control page and can accept or decline each one; acceptance stages the requested window and category for every selected area in shared desired state.
