...
- Public activation request form at `https://sua.actuallyleviticus.xyz/request`. A request records one or more selected areas, a separate RA1/RA2/RA3 category for each restricted area (danger areas need no RA category), one or more separate activation time slots entered in UTC/Z with the requester's local equivalent displayed underneath, the requester's name or VATSIM CID, contact email, and required activation details explaining why the airspace is needed without activating it automatically. RA1 is shown in green, RA2 in yellow, and RA3 in red. Controllers review pending requests from the `REQUESTS` button on the control page and can accept or decline each one; acceptance stages every requested slot and each restricted area's category, allowing the airspace to deactivate between slots and reactivate at the next one.
...
- Ten minutes before a timed NOTAM, request, or website/manual schedule deactivates, the plugin checks whether the SUA boundary and altitude band intersect the connected controller's live sector. HJ/HN solar schedules continue activating and deactivating automatically without notifications. One native vatSys-styled popup below the menu groups every qualifying area by deactivation time and lets that controller either allow the scheduled deactivations or retain all listed areas as their own H24 vatSys activations. Retained airspace follows the normal `USER: CID` path, is user-locked on the website, and remains editable/deactivatable through the controller's native Restricted Area window.
...
- Controller-created activations are additive and never delete or replace stored manual, request, solar, or NOTAM schedules. The website prioritizes `USER: CID`/`USER LOCKED` while the controller source is live; if that controller deactivates before an underlying active schedule ends, the scheduled activation takes over again.
...
- The internal manual-source type is not displayed as a `MAN` badge. A live vatSys controller activation displays only its timing state (`ACTIVE`, `PREACT`, or `OFF`) and `USER: CID`; overlapping stored sources do not add `SCHED` or `SAVED` badges to that row.
