...
- Public activation request form at `https://sua.actuallyleviticus.xyz/request`. A request records one or more selected areas, UTC start/end, requester, and optional notes without activating airspace. Controllers review pending requests from the `REQUESTS` button on the control page and can accept or decline each one; acceptance stages the requested window for every selected area in shared desired state.
