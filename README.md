...
- A future timed activation displays `DEACTIVE` with `SCHED`; once its window opens, `ACTIVE` replaces both. Stored-source implementation details are not shown as status tags.
...
- The internal manual-source type is not displayed as a `MAN` badge. A live vatSys controller activation displays only its timing state (`ACTIVE`, `PREACT`, or `OFF`) and `USER: CID`; overlapping stored sources do not add `SCHED` or other source badges to that row.
...
| `POST /api/sua/copy` | Copy levels, applicable RA category, and activation timing to selected SUA |
