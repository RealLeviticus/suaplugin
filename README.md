...
- Ten minutes before a NOTAM-scheduled area deactivates, the plugin checks whether its horizontal boundary and altitude band intersect the connected controller's live sector. A small top-right vatSys popup lets that controller either allow the scheduled deactivation or retain the area as their own H24 vatSys activation. Retained airspace follows the normal `USER: CID` path, is user-locked on the website, and remains editable/deactivatable through the controller's native Restricted Area window.
