...
- Public activation request form at `https://sua.actuallyleviticus.xyz/request`. A request records one or more selected areas, a separate RA1/RA2/RA3 category for each area, one or more separate activation time slots, the requester's name or VATSIM CID, contact email, and required activation details explaining why the airspace is needed without activating it automatically. RA1 is shown in green, RA2 in yellow, and RA3 in red. Controllers review pending requests from the `REQUESTS` button on the control page and can accept or decline each one; acceptance stages every requested slot and each area's category, allowing the airspace to deactivate between slots and reactivate at the next one.
