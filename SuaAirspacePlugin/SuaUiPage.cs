...
.row{display:grid;grid-template-columns:150px 46px 140px minmax(180px,1fr) 280px 140px;gap:8px;align-items:center;background:#8f9c9c;border:1px solid #5a6a6a;border-top:none;padding:7px 10px;font-size:13px;color:#101820;}
...
@media (max-width:760px){
  .row{grid-template-columns:minmax(0,1fr) 46px;grid-template-areas:'name type' 'status status' 'actions actions';grid-auto-rows:auto;}
...
(function(){
  ...
  function render(){
    ...
    for (var i = 0; i < areas.length; i++){
      ...
      html += ""<div class='row'>""
        ...
        + ""<span class='type'>"" + esc(a.Type.toUpperCase() === 'DANGER' ? 'DGR' : 'RST') + ""</span>""
  ...
  function renderNotams(notams){
    ...
    for (var i = 0; i < notams.length; i++){
      ...
      html += ""<div class='nt-card'>""
        ...
        + (n.Suppressed ? ""<span class='pill def'>DEACTIVATED</span>"" : ""<span class='pill sched'>AUTO SAVED</span>"") + ""</div>""
      ...
      html += ""<button onclick='activateNotam("" + n.Id + "",\""now\"")'>ACTIVATE NOW ("" + n.Matched.length + "")</button>"";
      html += ""<button class='danger-btn' onclick='deactivateNotam("" + n.Id + "")'>DEACTIVATE ALL ("" + n.Matched.length + "")</button>"";
      if (n.Windows.length)
        html += ""<button onclick='activateNotam("" + n.Id + "",\""schedule\"")'>SCHEDULE "" + n.Windows.length + "" TIMES</button>"";
      html += ""</div>"";
    }
    el.innerHTML = html;
  }

  window.activateNotam = function(id, mode){
    document.getElementById('ntStatus').textContent = 'ACTIVATING...';
    fetch('/api/sua/notams/activate?id=' + id + '&mode=' + mode, { method: 'POST' }).then(function(r){ return r.json(); }).then(function(d){
      document.getElementById('ntStatus').textContent = d.Success ? ((d.Mode === 'schedule' ? 'SCHEDULED ' : 'ACTIVATED ') + d.Activated.length) : '';
      setMsg(d.Success ? '' : (d.Error || 'NOTAM activation failed.'));
      refresh(true);
    }).catch(function(e){ document.getElementById('ntStatus').textContent = ''; setMsg(e.message || 'NOTAM activation failed.'); });
  };

  window.deactivateNotam = function(id){
    document.getElementById('ntStatus').textContent = 'DEACTIVATING...';
    fetch('/api/sua/notams/deactivate?id=' + id, { method: 'POST' }).then(function(r){ return r.json(); }).then(function(d){
      document.getElementById('ntStatus').textContent = d.Success ? ('DEACTIVATED ' + d.Deactivated.length) : '';
      setMsg(d.Success ? '' : (d.Error || 'NOTAM deactivation failed.'));
      refresh(true);
      scanNotams();
    }).catch(function(e){ document.getElementById('ntStatus').textContent = ''; setMsg(e.message || 'NOTAM deactivation failed.'); });
  };
  ...
  function setFilter(f, btn){
    filter = f;
    ['fAll','fDanger','fRestricted'].forEach(function(id){ document.getElementById(id).className = ''; });
    btn.className = 'on';
    render();
  }
